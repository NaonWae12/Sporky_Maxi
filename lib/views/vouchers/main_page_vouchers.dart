  ...
  Future<void> _fetchVouchers() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        debugPrint(
          '[VoucherPage] ❌ Failed to fetch vouchers (${res.statusCode})',
        );
        if (mounted) {
          setState(() {
...
class _VoucherCardItem extends StatelessWidget {
  ...
  void _showDetailBottomSheet(BuildContext context) {
    showAppBottomSheet<void>(
      ...
      child: _VoucherDetailSheet(uuid: data['uuid']?.toString() ?? ''),
  ...
  Widget _buildStatusBadge(String status, bool isExpired) {
    ...
    return Container(
      ...
      child: Text(label, style: AppTextStyles.list3Bold(text)),
    );
  }
          ...
          children: [
            if (_isLoading)
              ...
              if (_detail?['code'] != null)
                GlobalsButton(
                  color: AppColors.primary1,
                  textColor: AppColors.base1,
                  onPressed: () => _copyCode(_detail!['code'].toString()),
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.copy, size: 16, color: AppColors.base1),
                      SizedBox(width: 8),
                      Flexible(
                        child: GlobalsButtonText(
                          text: 'Salin Kode Voucher',
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 8),
          Expanded(
            child: Text(value, style: AppTextStyles.list1Bold(AppColors.base1)),
          ),
        ],
      ),
    );
